...
import time
import pandas
import treeinterpreter.treeinterpreter
...
def model_for_identifying_histology(overlayPath, dataCubePath, kFoldIndicesPath, idModelBegin, modelPath, makePermutationImportances, makeTreeInterpreter, folderPermutationImportances, treeInterpreterPath, permutationImportancesList):
    """
    ...
    if "random_state" in additionalParametersNoiseReduction:
        additionalParametersNoiseReduction["random_state"] = randomState
    
    if "random_state" in additionalParametersSavitzkyGolayFilter:
        additionalParametersSavitzkyGolayFilter["random_state"] = randomState
    
    if "random_state" in additionalParametersClassesEqualization:
        additionalParametersClassesEqualization["random_state"] = randomState
    
    if "random_state" in additionalParametersFeatureSelection:
        additionalParametersFeatureSelection["random_state"] = randomState
    
    for additionalParametersPermutationImportances, methodClassesEqualization, additionalParametersClassesEqualization in permutationImportancesList:
        if "random_state" in additionalParametersPermutationImportances:
            additionalParametersPermutationImportances["random_state"] = randomState
    ...
    if escape:
        ...
        #continue
        print("The model parameters are not valid")
        return
    ...
    if (widthSubwindow > 1) or (heightSubwindow > 1):
        trainingSetInput = finalTrainingSetInput
        trainingSetOutput = finalTrainingSetOutput
        
        testSetInput = finalTestSetInput
        testSetOutput = finalTestSetOutput
    
    
    if makePermutationImportances:
        if (folderPermutationImportances != ".") and (folderPermutationImportances != ""):
            os.makedirs(folderPermutationImportances, exist_ok = True)
        
        
        for additionalParametersPermutationImportances, methodClassesEqualization, additionalParametersClassesEqualization in permutationImportancesList:
            
            print("computing permutation importances for the training set")
            
            resultsPermutationImportance = sklearn.inspection.permutation_importance(model, trainingSetInput, trainingSetOutput, **additionalParametersPermutationImportances)
            
            fileName = os.path.join(folderPermutationImportances, str(idModel) + "_training_set.pkl")
            
            with open(fileName, "wb") as f:
                pickle.dump(resultsPermutationImportance, f)
            
            
            if methodClassesEqualization is None:
                
                print("computing permutation importances for the test set")
                
                resultsPermutationImportance = sklearn.inspection.permutation_importance(model, testSetInput, testSetOutput, **additionalParametersPermutationImportances)
            
            else:
                
                print("classes equalization in progress")
                
                sampler = methodClassesEqualization(**additionalParametersClassesEqualization)
                testSetInputResampled, testSetOutputResampled = sampler.fit_resample(testSetInput, testSetOutput)
                
                
                print("computing permutation importances for the test set")
                
                resultsPermutationImportance = sklearn.inspection.permutation_importance(model, testSetInputResampled, testSetOutputResampled, **additionalParametersPermutationImportances)
            
            
            fileName = os.path.join(folderPermutationImportances, str(idModel) + "_test_set.pkl")
    ...
    if makeTreeInterpreter:
        
        print("interpreting the tree model")
        
        if os.path.dirname(treeInterpreterPath) != "":
            os.makedirs(os.path.dirname(treeInterpreterPath), exist_ok = True)
        
        predictions, bias, contributions = treeinterpreter.treeinterpreter.predict(model, testSetInput)
        
        treeInterpreterInformations = TreeInterpreterInformations(predictions, bias, contributions)
        
        with open(treeInterpreterPath, "wb") as f:
            pickle.dump(treeInterpreterInformations, f)




class TreeInterpreterInformations:
    
    def __init__(self, predictions, bias, contributions):
...
if __name__ == "__main__":
    ...
    MAKEPERMUTATIONIMPORTANCES = True
    MAKETREEINTERPRETER = False
    
    FOLDERPERMUTATIONIMPORTANCES = "./results_model_analysis_3"
    TREEINTERPRETERPATH = "./results_model_analysis_3/tree_interpreter_results.pkl"
    
    PERMUTATIONIMPORTANCESLIST = [({"scoring" : "f1_weighted", "n_repeats" : 5, "n_jobs" : -1, "random_state" : RANDOMSTATE}, imblearn.under_sampling.RandomUnderSampler, {"random_state" : RANDOMSTATE}), ({"scoring" : "f1_macro", "n_repeats" : 5, "n_jobs" : -1, "random_state" : RANDOMSTATE}, imblearn.under_sampling.RandomUnderSampler, {"random_state" : RANDOMSTATE})]
    
    
    model_for_identifying_histology(OVERLAYPATH, DATACUBEPATH, KFOLDINDICESPATH, IDMODELBEGIN, MODELPATH, MAKEPERMUTATIONIMPORTANCES, MAKETREEINTERPRETER, FOLDERPERMUTATIONIMPORTANCES, TREEINTERPRETERPATH, PERMUTATIONIMPORTANCESLIST)
