...
def create_set_subwindows(setInput, setOutput, setFileNumbers, setPositions, completeSetInput, completeSetOutput, completeSetFileNumbers, completeSetPositions, widthSubwindow, heightSubwindow, methodSubwindow):
    ...
    cdef Py_ssize_t numberColumnsInputSets = completeSetInput.shape[1]
    ...
    finalSetInput = numpy.zeros((setInput.shape[0], completeSetInput.shape[1] * widthSubwindow * heightSubwindow), dtype = numpy.single)
...
def augment_data_image_transformations(setInput, setOutput, sizeSubwindow, methodSubwindow, makeMirrors, makeRotations):
    setInput = numpy.ascontiguousarray(setInput.astype(numpy.single))
    setOutput = numpy.ascontiguousarray(setOutput.astype(numpy.intc))
    
    if makeMirrors:
        setInput, setOutput = augment_data_mirrors(setInput, setOutput, sizeSubwindow, methodSubwindow)
    
    if makeRotations:
        setInput, setOutput = augment_data_rotations(setInput, setOutput, sizeSubwindow, methodSubwindow)
...
def augment_data_mirrors(setInput, setOutput, sizeSubwindow, methodSubwindow):
    setInputAugmented = numpy.zeros((3 * setInput.shape[0], setInput.shape[1]), dtype = numpy.single)
    
    if methodSubwindow == "subwindow_center":
        setOutputAugmented = numpy.zeros((3 * setInput.shape[0],), dtype = numpy.intc)
        setOutputAugmentedMultioutput = numpy.zeros((1, 1), dtype = numpy.intc)
        
        setOutput2 = setOutput
        setOutput2Multioutput = numpy.zeros((1, 1), dtype = numpy.intc)
    
    
    elif methodSubwindow == "subwindow_multioutput":
        setOutputAugmentedMultioutput = numpy.zeros((3 * setInput.shape[0], sizeSubwindow * sizeSubwindow), dtype = numpy.intc)
        setOutputAugmented = numpy.zeros((1,), dtype = numpy.intc)
        
        setOutput2Multioutput = setOutput
        setOutput2 = numpy.zeros((1,), dtype = numpy.intc)
    
    
    #setFileNumbersAugmented = numpy.zeros((3 * setInput.shape[0],), dtype = numpy.intc)
    #setPositionsAugmented = numpy.zeros((3 * setInput.shape[0], 2), dtype = numpy.intc)
    
    
    cdef float[:, ::1] setInputAugmentedView = setInputAugmented
    
    cdef int[::1] setOutputAugmentedView = setOutputAugmented
    cdef int[:, ::1] setOutputAugmentedMultioutputView = setOutputAugmentedMultioutput
    
    #cdef int[::1] setFileNumbersAugmentedView = setFileNumbersAugmented
    #cdef int[:, ::1] setPositionsAugmentedView = setPositionsAugmented
    
    
    cdef float[:, ::1] setInputView = setInput
    
    cdef int[::1] setOutputView = setOutput2
    cdef int[:, ::1] setOutputMultioutputView = setOutput2Multioutput
    ...
    cdef int numberSpectra = setInput.shape[0]
    cdef int numberFeatures = setInput.shape[1]
    cdef int sizeSpectra = setInput.shape[1] // (sizeSubwindow * sizeSubwindow)
    #cdef int widthSubwindowCVariable = widthSubwindow
    #cdef int heightSubwindowCVariable = heightSubwindow
    cdef int sizeSubwindowCVariable = sizeSubwindow
    cdef int numberOutputs = sizeSubwindow * sizeSubwindow
    cdef int indexNewSpectra = 0
    cdef int indexOrigin = 0
    cdef int indexDestination = 0
    ...
    while i < numberSpectra:
        j = 0
        while j < numberFeatures:
            setInputAugmentedView[indexNewSpectra, j] = setInputView[i, j]
            
            j += 1
        
        
        if methodSubwindowCode == 0:
            setOutputAugmentedView[indexNewSpectra] = setOutputView[i]
        
        if methodSubwindowCode == 1:
            j = 0
            while j < numberOutputs:
                setOutputAugmentedMultioutputView[indexNewSpectra, j] = setOutputMultioutputView[i, j]
                
                j += 1
        
        
        #setFileNumbersAugmentedView[indexNewSpectra] = setFileNumbersView[i]
        ...
        j = 0
        while j < sizeSubwindowCVariable:
            k = 0
            while k < sizeSubwindowCVariable:
                indexOrigin = (j * sizeSubwindowCVariable) + k
                indexDestination = ((sizeSubwindowCVariable - 1 - j) * sizeSubwindowCVariable) + k
                
                l = 0
                while l < sizeSpectra:
                    setInputAugmentedView[indexNewSpectra, indexDestination * sizeSpectra + l] = setInputView[i, indexOrigin * sizeSpectra + l]
                ...
                #elif methodSubwindowCode == 1:
                if methodSubwindowCode == 1:
                    setOutputAugmentedMultioutputView[indexNewSpectra, indexDestination] = setOutputMultioutputView[i, indexOrigin]
        ...
        while j < sizeSubwindowCVariable:
            ...
            while k < sizeSubwindowCVariable:
                indexOrigin = (j * sizeSubwindowCVariable) + k
                indexDestination = (j * sizeSubwindowCVariable) + (sizeSubwindowCVariable - 1 - k)
    ...
    if methodSubwindow == "subwindow_multioutput":
        return setInputAugmented, setOutputAugmentedMultioutput
    
    
    return setInputAugmented, setOutputAugmented




def augment_data_rotations(setInput, setOutput, sizeSubwindow, methodSubwindow):
    setInputAugmented = numpy.zeros((4 * setInput.shape[0], setInput.shape[1]), dtype = numpy.single)
    
    if methodSubwindow == "subwindow_center":
        setOutputAugmented = numpy.zeros((4 * setInput.shape[0],), dtype = numpy.intc)
        setOutputAugmentedMultioutput = numpy.zeros((1, 1), dtype = numpy.intc)
        ...
        setOutputAugmentedMultioutput = numpy.zeros((4 * setInput.shape[0], sizeSubwindow * sizeSubwindow), dtype = numpy.intc)
        setOutputAugmented = numpy.zeros((1,), dtype = numpy.intc)
    ...
    while i < numberSpectra:
        ...
        while m < 3:
            ...
            while j < sizeSubwindowCVariable:
                ...
                while k < sizeSubwindowCVariable:
                    indexOrigin = (j * sizeSubwindowCVariable) + k
                    indexDestination = (k * sizeSubwindowCVariable) + (sizeSubwindowCVariable - 1 - j)
                    
                    l = 0
                    while l < sizeSpectra:
                        setInputAugmentedView[indexNewSpectra, indexDestination * sizeSpectra + l] = setInputAugmentedView[indexNewSpectra - 1, indexOrigin * sizeSpectra + l]
                        
                        l += 1
                    
                    #if methodSubwindowCode == 0:
                    #    setOutputAugmentedView[indexNewSpectra] = setOutputAugmentedView[indexNewSpectra]
                    
                    #elif methodSubwindowCode == 1:
                    if methodSubwindowCode == 1:
                        setOutputAugmentedMultioutputView[indexNewSpectra, indexDestination] = setOutputAugmentedMultioutputView[indexNewSpectra - 1, indexOrigin]
            ...
            if methodSubwindowCode == 0:
                setOutputAugmentedView[indexNewSpectra] = setOutputAugmentedView[indexNewSpectra - 1]
