...
if __name__ == "__main__":
    ...
    for identifier in IDSTOTAKE:
        ...
        stringResult += ALGORITHMLIST[i] + " & " + str(round(get_f1_score_epithelium(dataFrame) * 100, 2)) + " & " + str(round(get_f1_score_stroma(dataFrame) * 100, 2)) + " & " + str(round(get_f1_score_blood(dataFrame) * 100, 2)) + " & " + str(round(get_f1_score_necrosis(dataFrame) * 100, 2)) + " & " + str(round(get_f1_score_weighted_average(dataFrame) * 100, 2)) + " \\\\\n\\hline\n"
