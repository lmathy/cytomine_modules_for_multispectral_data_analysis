...
if __name__ == "__main__":
    ...
    for identifier in IDSTOTAKE:
        ...
        stringResult += ALGORITHMLIST[i] + " & " + str(round(get_precision_epithelium(dataFrame) * 100, 2)) + " & " + str(round(get_precision_stroma(dataFrame) * 100, 2)) + " & " + str(round(get_precision_blood(dataFrame) * 100, 2)) + " & " + str(round(get_precision_necrosis(dataFrame) * 100, 2)) + " & " + str(round(get_precision_weighted_average(dataFrame) * 100, 2)) + " \\\\\n\\hline\n"
